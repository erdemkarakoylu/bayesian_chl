...
This section outlines the dataset, model structures, inference methods, and model evaluation criteria used in this study. I focus on a fully Bayesian approach to modeling chlorophyll-a from satellite remote sensing reflectance (Rrs), emphasizing transparency, uncertainty quantification, and principled model comparison. Four representative models are presented in the main text, while all seven model formulations and results are included in the Supplement.

### 2.1 Study Area and Dataset

For pedagogical reasons, I used the well known NOMAD [@werdell2005improved] dataset, which includes quality-controlled in situ measurements of chlorophyll-a matched with coincident satellite Rrs data. These measurements span a range of oceanographic conditions, enabling the development of models that generalize across water types. Sea-surface reflectances (Rrs) in the 6 visible SeaWiFS bands (411, 443, 489, 510, 550, 670nm) constitue the bulk of the data. As prediction targets corresponding chlorophyll concentrations using both fluorescence and HPLC were retained. 

To prepare the data:

- **Chlorophyll-a**:  
  - Measurements obtained using both HPLC and fluorescence methods  
  - Combined into a single log-transformed target variable (log₁₀ Chl)  
  - A binary flag identifies the measurement method for each sample  

- **Remote sensing reflectance (Rrs)**:  
  - Rrs values equal to zero were excluded  
  - A derived feature, the Maximum Band Ratio (MBR), was computed as:  
    - Numerator: max(Rrs411, Rrs443, Rrs489, Rrs510)  
    - Denominator: Rrs555 + Rrs670  
  - MBR was log-transformed to create the primary input variable  

- **Grouping and flags**:  
  - A categorical variable (`MBR_flag`) was used to indicate the dominant Rrs band used in the MBR numerator  
  - This grouping informed partial pooling structures in hierarchical models  

### 2.2 Bayesian Modeling Framework

All models were implemented in PyMC v5 and estimated using Hamiltonian Monte Carlo with the No-U-Turn Sampler (NUTS). I modeled log-transformed chlorophyll-a as a function of log-transformed MBR, using a truncated normal likelihood to ensure predictions remained within physically plausible bounds.

Key components of the modeling framework include:

- **Likelihood**:  
  - Truncated normal with support between –3.0 and 3.2 (log₁₀ Chl)  

- **Noise structure** (`σ`):  
  - Varied across models, with increasing structural complexity  
  - Some models use constant σ; others include heteroscedastic or hierarchical formulations  

- **Priors**:  
  - Weakly informative priors were used throughout  
  - Regression coefficients received Normal priors  
  - All σ terms were modeled with Gamma distributions, allowing flexible but strictly positive support  

- **Software**:  
  - PyMC for model specification and inference  
  - ArviZ for posterior diagnostics, posterior predictive checks, and model comparison  

### 2.3 Overview of Candidate Models

I developed seven Bayesian models representing a progression of modeling complexity and assumptions:
...
- **Model 3**: Hierarchical linear regression extended to include all Rrs spectral dominance groups  
- **Model 4**: Model 2 with group-specific constant σ  
- **Model 5**: Global linear heteroscedastic model  
- **Model 6**: Model 2 with group-wise linear heteroscedasticity  
- **Model 7**: Model 6 plus an added dispersion term for fluorescence-based measurements  

The main text focuses on Models 1, 2, 6, and 7, which span the range of key structural variations. Full equations, priors, and diagnostic results for all models are provided in the Supplement.

### 2.4 Representative Model Structures

#### 2.4.1 Model 1: Bayesian Polynomial OC6 Regression

This model follows the OC6 formulation proposed by O’Reilly et al. (2019), adapted to a Bayesian framework. It uses a fourth-order polynomial in log(MBR) to estimate log(Chl).

- No grouping or partial pooling  
- Constant noise term σ shared across all observations  
- Gamma prior placed on σ  

#### 2.4.2 Model 2: Hierarchical Linear Regression (4-band Grouping)

This model introduces partial pooling across four groups defined by the dominant MBR numerator band. Each group receives its own slope and intercept, with shared hyperpriors.

- Grouping: Rrs411, Rrs443, Rrs489, Rrs510  
- Partial pooling via shared hyperpriors  
- Constant σ across all groups  

#### 2.4.3 Model 6: Hierarchical Linear Regression with Group-wise Linear σ

To accommodate heteroscedasticity, this model allows σ to vary with log(MBR) within each group, modeled as a log-linear relationship. Parameters are estimated hierarchically across groups.

- log(σ) = α + β × log(MBR), with group-specific α and β  
- Transformed via exp(log(σ))  
- Partial pooling of σ slope/intercept terms  

#### 2.4.4 Model 7: Hierarchical Linear σ + Measurement-Specific Dispersion

This model adds a measurement-method-specific dispersion term to the structure of Model 6, capturing the known increase in variability associated with fluorescence-based chlorophyll estimates.

- Additional noise component applied when chl_flag = fluorescence  
- All other structure inherited from Model 6  

### 2.5 Prior Specification and Inference Settings

I used weakly informative priors across all models to balance flexibility and regularization:

- **Regression terms**: Normal(0, 1) unless otherwise stated  
- **Hyperpriors**: Normal and HalfNormal for hierarchical parameters  
- **Noise terms** (`σ`): Gamma(γ, φ), chosen to reflect plausible dispersion values  

Inference was conducted using:

- 4 chains per model  
- 1000–1500 tuning steps  
- ≥2000 posterior samples per chain  

I assessed convergence using R-hat (target < 1.01) and effective sample size (ESS > 1000). Full trace plots and diagnostics are included in the Supplement.

### 2.6 Model Comparison and Predictive Evaluation

I compared model performance using **Pareto Smoothed Importance Sampling Leave-One-Out Cross-Validation (PSIS-LOO-CV)**, implemented via the `arviz.loo()` function. This Bayesian metric estimates the expected log predictive density (ELPD), a principled measure of out-of-sample predictive accuracy.
