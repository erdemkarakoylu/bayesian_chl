...
*Model 2* introduces partial pooling across four groups defined by the dominant MBR numerator band; Rrs411, Rrs443, Rrs489, Rrs510. Each group receives its own slope and intercept with shared hyperpriors. On the one hand each group has its own submodel that can capture details specific to it, while the shared hyperpriors allow for the sharing of information between groups when fitting the model, thus maximizing the use of information contained in the available data. $X_i$ is the same as for Model 1, i.e. $log(MBR)$, assigned a given observation to group $i$ (@fig-eda-mbr). This model retains a constant dispersion parameter $σ$, similar to *Model 1*.
Below is the system of equations corresponding to this model, where $j$ is the group index and $i$ is the $i^{th}$ obasrvation. Mathematical description of the model can be found in the Supplementary Material. @fig-model2-struct illustrates the model structure.
...
The data plotted in @fig-eda-mbr suggests heteroscedasticity - variance that is not constant. Morevoer variance seems to change with the data. Models 3 and 4 (see Supplemental Material section) investigated the role of groupwise constant heteroscedasticity and global linear heteroscedasticity, respectively. To investigate this further, Model 5 include groupwise linear heteroscedasticity. Thus,  for a given MBR group $j$ $σ_j$ is modeled as a log-linear relationship such that $$log(σ) = α_j + β_j × log(MBR)$ and then transformed via $exp(log(σ_j))$ before being passed to the likelihood. Similarly to the model mean $μ_j$, $σ_j$'s slope and intercept terms are partially pooled and so have common hyperpriors. Mathematical descriiption of the model can be found in the Supplementary Material section; @fig-model5-struct.  illustrates the model structure.
...
Model 6 - Similar to Model 5; multi-level hierarchical linear model partial pooling across $MBR$  groups (Rrs411, Rrs443, Rrs489, Rrs510) for both model mean and variance. However the linear variance model gets an additional parameter $γ$ that allows for the model dispersion to depend on the chlorophyll measurement method (fluorescence or HPLC). While this doesn't affect future data prediction, for which ground truth mearuement are optional, it does allow the development of insight on the effect of chloroophyll measurement method on model ucertainty.
:::
...
- **Hyperpriors**: Normal and Exponential for hierarchical parameters  
