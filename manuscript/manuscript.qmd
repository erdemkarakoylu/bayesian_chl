...
A sound Bayesian modeling principle is to develop several models with increasing levels of complexity as insight is progressively built. I largely follow this approach with the exception that the first model is built from a known formulation, a 4th order polynomial. All models use $log(MBR)$ as main input and $log(chl)$ as target, some use $MBR\_flag$ as additional input, the last model also uses $hlpc_flag$. The following are key components shared across models.
...
- **Model 4**: Model 2 with group-specific constant likelihood variance  
